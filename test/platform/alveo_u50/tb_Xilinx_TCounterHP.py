    ...
    def __init__(self, parent, name, reset, q):
        ...
        for i in range(w):
            ...
            if (i == 0):
                ...
                py4hw.Buf(self, 'nt{}'.format(i), r[i], t[i+1])
            else:
                ...
                for j in range(i):
                    ...
                    py4hw.Reg(self, 'd{}_{}'.format(i,j), last, nr)
                    if (j == i-1):
                        py4hw.And2(self, 'nt{}'.format(i), nr, r[0], t[i+1])
                    else:
                        nnr = self.wire('nt{}_{}'.format(i,j), last.getWidth())
                        py4hw.And2(self, 'nt{}_{}'.format(i,j), nr, r[i-j-1], nnr)
                        last = nnr
...
def testDesign(n, req_freq=None):
    ...
    TCounterHP(sys, 'counter', reset, q)
...
csv_file = 'data_xilinx_tcounter_hp.csv'
